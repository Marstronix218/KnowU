  ...
  return (
    <div className="setup-shell">
      <section className="setup-panel">
        <div className="setup-brand"><LogoMark /><strong>KnowU</strong></div>
        ...
        {step === 1 && (
          <div className="setup-content narrow">
            ...
            <p>Accessibility permission lets KnowU read the title of the focused window. It does not grant access to keystrokes, document bodies, screenshots, or the clipboard. Without it, app-duration tracking still works.</p>
        ...
        {step === 2 && (
          <div className="setup-content">
            ...
            <p>KnowU can temporarily inspect up to 90 days of selected history to build the first profile. Days 31–90 are deleted after that first profile succeeds.</p>
  ...
  return (
    <aside className="sidebar">
      <div className="brand-lockup">
        <LogoMark />
        <div>
          <div className="brand-name">KnowU</div>
  ...
  return (
    <div className="page now-page">
      <PageHeader
        ...
        description="KnowU reconstructs useful work threads from minimal local signals—without screenshots, audio, keystrokes, or page contents."
        actions={
          <>
...
function DashboardContent({ data }: { data: DashboardData }) {
  ...
  if (!selected) {
    return <EmptyState title="No work threads yet" detail="Keep collection on while you work. KnowU will group recent activity into reviewable threads." />;
  }
...
function ThreadsPage() {
  ...
  return (
    <div className="page threads-page">
      <PageHeader eyebrow="Reconstructed work" title="Your threads" description="KnowU groups related activity into provisional work streams. Review the evidence before treating an inference as intent." actions={<RangePicker value={range} onChange={setRange} />} />
      <ResourceState {...resource}>
        {(data) => {
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        description="Exactly what KnowU recorded—no hidden content, no reconstructed stories."
...
function ProfilePage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (profile.memorySync) {
      setSyncError(profile.memorySync.status !== "everos");
      setSyncMessage(profile.memorySync.message);
    }
    setLabel("");
    setDescription("");
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        title="What KnowU remembers"
        description="Only approved profile facts and corrections can be synced to EverOS. Raw activity always stays local."
        actions={(
          <div className="inline-actions">
            <button className="ghost-button" onClick={() => {
              setSyncMessage("Syncing approved profile memories to EverOS…");
              setSyncError(false);
              void api.syncProfileMemories()
                .then((receipt) => setSyncMessage(receipt.message))
                .catch((cause) => {
                  setSyncError(true);
                  setSyncMessage(cause instanceof Error ? cause.message : String(cause));
                });
            }}><RefreshCw size={16} /> Sync approved profile</button>
            <button className="primary-button" onClick={() => { setEditingId(undefined); setLabel(""); setDescription(""); setShowForm(true); }}><Plus size={16} /> Add correction</button>
          </div>
        )}
      />
      {syncMessage && <p className={syncError ? "error-message" : "success-message"}>{!syncError && <Check size={14} />}{syncMessage}</p>}
      <ResourceState {...resource}>
        {(profile) => (
      ...
      {showForm && (
        <Modal title={editingId ? "Edit authoritative correction" : "Add authoritative correction"} onClose={() => setShowForm(false)}>
          <form className="stack-form" onSubmit={(event) => void submit(event)}>
            <label>What should KnowU know?<input required value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Privacy is more important to me than adding features" /></label>
...
function AssistantPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setRunIntegration(result.integration);
    } catch (cause) {
    ...
    <div className="page assistant-page">
      ...
      <div className="assistant-workspace">
        ...
        <ContextEconomicsPanel
          ...
          onSeed={() => {
            setSeedMessage("Seeding three approved demo memories in EverOS…");
            void api.seedDemoMemories()
              .then((receipt) => setSeedMessage(receipt.message))
              .catch((cause) => setSeedMessage(cause instanceof Error ? cause.message : String(cause)));
          }}
        />
      </div>
    </div>
...
function ContextEconomicsPanel({
  ...
  runIntegration?: { memoryProvider: string; memoryWarning?: string; telemetryStatus: string };
  seedMessage: string;
  onSeed: () => void;
}) {
  return (
    <aside className="economics-panel" aria-label="Context Economics">
      ...
      {economics ? (
        <>
          ...
          <section className="remembered-list">
            <div className="economics-section-title"><Brain size={15} /><span>KnowU remembered</span></div>
      ...
      <div className="integration-status">
        <article><span className={integration?.everos.configured ? "status-light" : "status-light offline"} /><div><strong>EverOS memory</strong><small>{integration?.everos.message ?? "Checking configuration…"}</small></div></article>
        <article><span className={integration?.snowflake.configured ? "status-light" : "status-light offline"} /><div><strong>Snowflake analytics</strong><small>{integration?.snowflake.message ?? "Checking configuration…"}</small></div></article>
        <button className="ghost-button" onClick={onSeed}><Sparkles size={15} /> Seed demo memories</button>
        ...
        {(settings) => (
          <div className="settings-grid">
            ...
            <section className="panel settings-card full-width danger-card">
              <SettingsHeading icon={<Trash2 />} title="Delete local KnowU data" detail="Permanently removes local activity, profiles, corrections, recommendations, telemetry, settings, and provider credentials." />
