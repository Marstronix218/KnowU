...
pub struct AppState {
    ...
    pub memory: EverOSMemoryService,
    pub analytics: SnowflakeAnalyticsService,
...
fn fetch_website_icon(page_url: &str) -> Option<String> {
    ...
    let response = client
        .get(page_url.clone())
        .header(reqwest::header::USER_AGENT, "KnowU/0.2 favicon")
        .send()
        .ok()?
        .error_for_status()
        .ok()?;
...
pub async fn save_profile_correction(
    ...
    let sync = if state.memory.is_configured() {
        match state
            .memory
            .store_memory(&correction_memory(&correction))
            .await
        {
            Ok(receipt) => json!({"status":"everos","message":receipt.message}),
            Err(error) => json!({
                "status":"local-only",
                "message":format!("Saved locally. EverOS sync failed: {error}")
            }),
        }
    } else {
        json!({
            "status":"local-only",
            "message":"Saved locally. Set EVEROS_API_KEY to sync approved memories to EverOS."
        })
    };
    profile["memorySync"] = sync;
    Ok(profile)
}

#[tauri::command]
pub async fn sync_profile_memories(state: State<'_, AppState>) -> AppResult<Value> {
    if !state.memory.is_configured() {
        return Err(AppError::InvalidInput(
            "EverOS is not configured. Set EVEROS_API_KEY and restart KnowU.".into(),
        ));
    }
    let memories = approved_profile_memories(&state.db.profile()?, &state.db.corrections()?);
    let receipt = state.memory.store_memories(&memories).await?;
    Ok(json!({
        "provider":receipt.provider,
        "storedCount":receipt.stored_count,
        "message":receipt.message
    }))
}

#[tauri::command]
pub async fn seed_demo_memories(state: State<'_, AppState>) -> AppResult<Value> {
    if !state.memory.is_configured() {
        return Err(AppError::InvalidInput(
            "EverOS is not configured. Set EVEROS_API_KEY before seeding demo memories.".into(),
        ));
    }
    let memories = demo_seed_memories();
    let receipt = state.memory.store_memories(&memories).await?;
    Ok(json!({
        "provider":receipt.provider,
        "storedCount":receipt.stored_count,
...
#[tauri::command]
pub fn get_integration_status(state: State<'_, AppState>) -> Value {
    json!({
        "everos":{
            "configured":state.memory.is_configured(),
            "message":state.memory.configuration_message()
        },
        "snowflake":{
            "configured":state.analytics.is_configured(),
            "message":state.analytics.configuration_message(),
            "promptTokenizationEnabled":state.analytics.prompt_tokenization_enabled()
        }
    })
}
...
pub async fn chat(
    ...
    let measurement =
        if state.analytics.is_configured() && state.analytics.prompt_tokenization_enabled() {
            state
                .analytics
                .measure_tokens(&baseline_prompt, &optimized_prompt)
                .await
                .unwrap_or_else(|_| {
    ...
    let (snowflake_synced, analytics_error, telemetry_status) = if state.analytics.is_configured() {
        match state.analytics.record_inference_run(&run).await {
            Ok(()) => (true, None, "synced-to-snowflake".to_string()),
            Err(error) => (
                false,
                Some(error.to_string()),
                format!("local-only: Snowflake sync failed: {error}"),
            ),
        }
    } else {
        (
            false,
            Some("Snowflake is not configured.".into()),
            "local-only: Snowflake credentials missing".into(),
        )
    };
    state
        .db
        .record_inference_run(&run, snowflake_synced, analytics_error.as_deref())?;
    Ok(json!({
        "message":{
