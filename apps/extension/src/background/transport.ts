...
async function authenticatedFetch(
  ...
  try {
    ...
    throw new TransportError("The local KnowU app could not be reached.", "connection");
  } finally {
...
function sendNativeRequest(request: NativeRequest): Promise<NativeResponse> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new TransportError(
          "The KnowU Native Messaging host did not respond.",
          "connection"
        )
      );
    }, REQUEST_TIMEOUT_MS);
    ...
    chrome.runtime.sendNativeMessage(
      ...
      (response: NativeResponse | undefined) => {
        ...
        if (chrome.runtime.lastError) {
          reject(
            new TransportError(
              "The KnowU Mac app or Native Messaging host is unavailable.",
              "connection"
            )
          );
          return;
...
export async function testConnection(
  ...
  try {
    ...
    await saveConnection({
      state: "connected",
      message: "Connected to the local KnowU app",
