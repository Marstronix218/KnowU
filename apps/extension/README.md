...
paired KnowU app on the same computer.

## Screenshot

![KnowU Chrome companion settings for Native Messaging pairing](../../docs/screenshots/chrome-companion-settings.jpg)

## Prerequisites

The companion is useful only with the native KnowU desktop app running on the
...
downloaded and installed KnowU yet, follow
[Local development](../../README.md#local-development) first.

## Build, load, and pair

If you installed KnowU from a bundle but do not have the repository locally,
...
2. Enable **Developer mode**, choose **Load unpacked**, and select
   `apps/extension/dist`.
3. Copy the 32-character ID shown on the KnowU extension card.
...
KnowU to observe; registration can be reused when Chrome shows the same
...
Select the KnowU toolbar icon to open the popup. It shows:
...
- **The desktop app is unavailable:** live timing cannot be delivered, but
  KnowU can still use imported Chrome history after the desktop app starts.
  ...
  "events": [
    {
      ...
      "source": "chrome_extension",
      "browser": "chrome",
      "browserProfileId": "<approved Chrome profile ID from KnowU Settings>",
