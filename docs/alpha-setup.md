...
Allow the running KnowU development process when prompted. Without
...
Permission does not give KnowU a feature for recording keys, screenshots,
...
entered key. Do not commit keys. KnowU does not automatically read `.env`
files.
...
Preferred: open KnowU **Settings → Chrome companion pairing**, paste the
...
pairing token. It removes KnowU's Native Messaging manifest but does not
